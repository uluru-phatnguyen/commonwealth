...
const LinkNewAddressModal: m.Component<{
  ...
  step,
  error,
  selectedWallet: LinkNewAddressWallets,  // select wallet step
  validSig: string,                       // enter address step
  secretPhraseSaved: boolean,             // enter address step
  newAddress: Account<any>,               // enter address step (true if account was initialized, otherwise Account)
  isNewLogin: boolean,                    // create profile step
  hasName: boolean,
  hasHeadline: boolean,
  ...
  view: (vnode) => {
    ...
    return m('.LinkNewAddressModal', [
      vnode.state.step === LinkNewAddressSteps.Step1SelectWallet ? m('.link-address-step', [
        ...
        m('p.link-address-precopy', !app.user ? [
          'Select a wallet:'
